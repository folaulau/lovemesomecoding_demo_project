...
async function seed() {
  ...
  `);
    ...
    for (const spec of PROFILES) {
        const saved = await profileRepo.save(profileRepo.create({
        ...
        await imageRepo.save(spec.portfolio.map((caption, index) => imageRepo.create({
    ...
    const waterHeater = await projectRepo.save(projectRepo.create({
        homeownerId: users.get('maya').id,
        categoryId: categories.get('plumbing').id,
        title: 'Replace leaking 50-gallon water heater',
        description: 'The water heater in the garage is weeping from the base and there is rust on the pan. It is the original unit from when the house was built in 2009, so I am assuming replacement rather than repair. Gas, 50 gallon, garage install with decent access.',
        city: 'Austin',
        state: 'TX',
        zip: '78704',
        budgetMin: 1800,
    ...
    await quoteRepo.save([
        quoteRepo.create({
            ...
            amount: 2450,
            estimatedDays: 1,
            message: 'Price covers a 50-gallon gas unit, new expansion tank, drain pan and permit. Same-day if you can be home before noon — I keep two units on the truck.',
        ...
        quoteRepo.create({
            ...
            amount: 2890,
            estimatedDays: 2,
            message: 'Recommending a tankless swap rather than like-for-like. Higher up front, but your gas line is already sized for it and you stop paying to keep 50 gallons hot overnight.',
            status: QuoteStatus.PENDING,
        }),
    ]);
    const lighting = await projectRepo.save(projectRepo.create({
        homeownerId: users.get('maya').id,
        categoryId: categories.get('electrical').id,
        title: 'Install recessed lighting in living room',
        description: 'Living room has one central fixture and is dark at the edges. Looking for six to eight recessed lights on a dimmer. Attic above is accessible. Would like the existing fixture removed and the box patched.',
        city: 'Austin',
        state: 'TX',
        zip: '78704',
        budgetMin: 1500,
    ...
    await quoteRepo.save([
        quoteRepo.create({
            ...
            amount: 1850,
            estimatedDays: 2,
            message: 'Eight 6" LED cans on two dimmable circuits, plus patching and texture. Ceiling is 1970s blown-in insulation, so I have priced in the extra containment.',
    ...
    const trim = await projectRepo.save(projectRepo.create({
        homeownerId: users.get('maya').id,
        categoryId: categories.get('painting').id,
        title: 'Repaint exterior trim and soffits',
        description: 'Cedar trim and soffits all round, single storey, roughly 1,900 sq ft footprint. The paint is failing on the south and west faces. Body of the house is brick and does not need anything.',
        city: 'Austin',
        state: 'TX',
        zip: '78704',
        budgetMin: 2500,
    ...
    const trimQuote = await quoteRepo.save(quoteRepo.create({
        projectId: trim.id,
        contractorId: profiles.get('trueline').id,
        amount: 3200,
        estimatedDays: 4,
        message: 'Scrape, spot-prime the bare cedar and two finish coats. The south face will need the most work — that is where the sun has taken the old coat off.',
        status: QuoteStatus.ACCEPTED,
    }));
    const roof = await projectRepo.save(projectRepo.create({
        homeownerId: users.get('daniel').id,
        categoryId: categories.get('roofing').id,
        title: 'Roof inspection after hail storm',
        description: 'We took golf-ball hail two weeks ago. No visible leak inside but the insurer wants an inspection report before they will open a claim. Two-storey, composition shingle, roughly 12 years old.',
        city: 'Round Rock',
        state: 'TX',
        zip: '78664',
        budgetMin: 200,
    ...
    await quoteRepo.save(quoteRepo.create({
        projectId: roof.id,
        contractorId: profiles.get('summit').id,
        amount: 350,
        estimatedDays: 1,
        message: 'Inspection fee is credited against any repair work you decide to go ahead with. You get photos of every slope and a written report either way.',
        status: QuoteStatus.PENDING,
    }));
    ...
    await reviewRepo.save(reviewRepo.create({
        ...
        rating: 5,
        comment: 'Marco found rot in two soffit boards that I did not know about, showed me the photos, and replaced them for the cost of the lumber rather than repainting over it. Four days, cleaned up every evening.',
    }));
    await AppDataSource.query(`
...
function placeholderUrl(seed, label) {
