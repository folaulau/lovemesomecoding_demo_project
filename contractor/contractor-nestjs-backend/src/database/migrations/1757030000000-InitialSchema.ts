...
/**
 * The whole schema, written by hand.
 *
 * `typeorm migration:generate` would have produced this by diffing the entities, and on a real
 * project that is what you should use. It is written out here because this repo's job is to be
 * read: generated migrations are one enormous line of `CREATE TABLE "projects" ("id" BIGSERIAL…`
 * with no comments, and the interesting decisions — why `numeric` and not `float`, why a partial
 * unique index — are exactly what that format destroys.
 *
 * ⚠️ Never edit a migration that has been applied anywhere. Postgres has already run it and the
 * `migrations` table has already recorded it, so an edit changes what NEW databases get and
 * nothing else — the two then differ forever, in a way nothing detects. Write another migration.
 */
export class InitialSchema1757030000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    /*
     * `gen_random_uuid()` is built into Postgres 13+, so no extension is needed. Older tutorials
     * reach for `uuid-ossp` and `uuid_generate_v4()`, which needs `CREATE EXTENSION` and therefore
     * superuser on many managed databases. This does not.
     */
    ...
    /* --------------------------------------------------------------------------------------- */
    /* contractor_services — which trades a pro works in                                         */
    /* --------------------------------------------------------------------------------------- */
    await queryRunner.query(`
    ...
    // ⚠️ The composite PK already indexes (profile, category) and therefore answers "what trades
    // does this pro work in". It does NOT answer "which pros work in this trade" — a B-tree can
    // only be scanned from its leading column. The directory's category filter asks exactly that
    // question, so it needs its own index.
    await queryRunner.query(`
    ...
    // The contractor lead feed is "open projects in these categories, newest first" and runs on
    // every page load. Equality columns lead, the sort column trails — reverse that and Postgres
    // has to sort the result instead of reading it in order.
    await queryRunner.query(`
    ...
    /*
     * ⚠️ A PARTIAL unique index, and the `WHERE` is the whole point.
     *
     * At most ONE quote per project may be accepted — that is rule 4 expressed as a constraint.
     * A plain `UNIQUE (project_id, status)` would also forbid a second DECLINED quote, which is
     * exactly what accepting one is supposed to produce. Restricting the index to the accepted
     * rows constrains the case that matters and ignores the rest.
     */
    await queryRunner.query(`
  ...
  /**
   * ⚠️ Dropped in reverse dependency order. Postgres refuses to drop a table another table still
   * references, so `users` cannot go before `projects`. `CASCADE` would paper over the ordering
   * and, on a real database, would take related objects with it that nobody meant to lose.
   */
