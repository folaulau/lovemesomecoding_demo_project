...
class CustomerOrderDAOIntegrationTest {
    ...
    void loadsOrderGraph() {
        CustomerOrder order = orderDAO.findByPublicIdWithItems(TestIds.ORDER_CUSTOMER_DELIVERY)
                .orElseThrow();
    ...
    void subtotalMatchesLineItems() {
        CustomerOrder order =
                orderDAO.findByPublicIdWithItems(TestIds.ORDER_MULTI_LINE).orElseThrow();
    ...
    void supportsGuestOrders() {
        CustomerOrder guestOrder =
                orderDAO.findByPublicId(TestIds.ORDER_GUEST_CARRYOUT).orElseThrow();
    ...
    void resolvesContactEmailFromUser() {
        CustomerOrder userOrder =
                orderDAO.findByPublicId(TestIds.ORDER_CUSTOMER_DELIVERY).orElseThrow();
